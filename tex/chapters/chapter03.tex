...
In this chapter, first, we give an introduction to cost-sensitive classification in 
...
Afterwards, we present the different cost-sensitive performance evaluation measures. Lastly, we show 
the state-of-the-art example-dependent cost-sensitive methods cost-proportionate rejection-sampling 
...
\label{sec:3:intro}
  ...
  \begin{remark}{Real-world example-dependent cost-sensitive applications}
...
misclassifying a negative example have a cost of $C_{FN}=0.2$ and a positive example $C_{FP}=1.8$. 
Under this scenario, misclassifying a positive example have a much higher cost than 
...
cost, of Algorithm3 and the classification algorithms presented in \figurename{~\ref{fig:2:2b}} 
...
\begin{remark}{Class-dependent cost-sensitive algorithms}
Over the past decades, various algorithms have been proposed for class-dependent cost-sensitive 
classification in literature. Several authors have used modifications of decision trees that take 
into account the different class-dependent costs 
...
Taking into account the example-dependent costs, in \figurename{~\ref{fig:3:3}} (Algorithm4), we the 
...
worth mentioning, how distinct are the results if the costs are ignored and an algorithm is 
