...
\subsection{Cost-sensitive ensembles}
  ...
  examples. Hence, these methods are not  well suited for example-dependent cost-sensitive 
  problems. 
...
\label{sec:9:theoretical}
  ...
  Let $H$ be an ensemble of $T>3$ classifiers $\mathcal{M}=\{M_1, M_2,\dots,M_T\}$, and 
 ...
 \noindent Similarly, for the set $\mathcal{S}_1$, the average classifier will have a correct 
  ...
  \noindent Finally, putting (\ref{eq:21}), (\ref{eq:22}), (\ref{eq:26}) and (\ref{eq:27}) 
  together, (\ref{eq:9:lemma}) is rewritten
  ...
  \noindent Since $\rho \ge \frac{1}{2}$ , then $P_c\ge\rho$ from (\ref{eq:9:Pc}), and using the 
  ...
  Let $H$ be an ensemble of $T>3$ classifiers $\mathcal{M}=\{M_1,\dots,M_T\}$, and $\mathcal{S}$ a 
