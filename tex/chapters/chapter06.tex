...
\hline
\multirow{2}{*}{$y_i$} & \multirow{2}{*}{$J_i(\theta)$} & 
\multicolumn{2}{c}{value of $J_i(\theta)$ if:} \\
& & $h_\theta(\mathbf{x}_i)) \approx 0$& $h_\theta(\mathbf{x}_i)) \approx 1$ \\
 \hline
 0 & $-\log(1-h_\theta(\mathbf{x}_i))$ & $\approx0$ & $\approx\infty $ \\
 1 & $-\log(h_\theta(\mathbf{x}_i))$ &  $\approx\infty $ & $\approx0$ \\
 \hline
\end{tabular}
\caption{Analysis of the logistic cost function}
\end{table}
