...
financial impact ranging from tens to twousands of Euros, similarly, approving a customer that does 
not rapays his loans have a significant impact on a bank profit.
...
\label{sec:4:fraud}
  ...
  dimensionality of the search space and how to preprocess the features
	\citep{Bolton2002,Gadi2008,Whitrow2008,DalPozzolo2014}.	We are interested to 
	address the cost-sensitivity and the features preprocessing 	issues. 
	
	\begin{remark}{Cost-sensitive}
	...
	depended on each 	transaction.	In \citep{CorreaBahnsen2013}, we proposed a new cost-based 
  ...
  approach do not take into account the spending behavior of the customer, which is expected to 
  help discover fraud patterns. A standard way to include these behavioral spending patters was 
  proposed in \citep{Whitrow2008}, they propose a transaction aggregation strategy in order to take 
  into account a customer spending behavior. The derivation of the aggregated features consists in 
  ...
  aggregation strategy, by incorporate a combination criteria when grouping the transaction. ie. 
  instead of aggregating only by card holder and transaction type, we combine it with country or 
  merchant group, this allow to have a much richer feature space. Moreover, there is still some 
  information that is not 	completely captured by the aggregated features. In particular we are 
  interesting in analyzing the 	time of the transaction. The logic behind this, is that a customer 
  is expected to make transactions at similar hours. We propose a new method for creating features 
  based in the periodic behavior of a transaction time, using the von Mises distribution. In 
  particular, the new features estimate if the time of a new transaction is within the confidence 
  interval of the previous transaction times \citep{CorreaBahnsen2015c}.
	Furthermore, we present the real credit card fraud dataset provided by a large European 
  card processing company, that we used for the experiments in this thesis.
...
\label{sec:4:frad:eval}
  ...
  Most of these studies compare they proposed algorithm with a benchmark algorithm and then make 
	...
	algorithm, we used the cost matrix as defined in \tablename{~\ref{tab:3:cost_matrix}}.
	...
	The costs are summarized in \tablename{~\ref{tab:4:table_costmat_fraud}}.

	Afterwards, using (\ref{eq:3:cost}) a cost measure fro fraud detection is calculated as:
  \begin{align}
...
\left(x_l^{id}=\mathbf{x}_i^{id}\right) \nonumber \\
		&\wedge \left(hours(\mathbf{x}_i^{time},x_l^{time}) < t_p\right) \bigg\}_{l=1}^N,
	\end{align}
	where $N=\vert\mathcal{S}\vert$, $\mathbf{x}_i^{time}$ is the time of transaction $i$,  
$\mathbf{x}_i^{amt}$ is the 
	amount of transaction $i$, $\mathbf{x}_i^{id}$  the customer identification number of transaction 
$i$, 
	...
	\[
		\mathbf{x}_i^{a1} = \vert TRX_{agg}(\mathcal{S},i, t_p) \vert \quad  \text{and} \quad 
\mathbf{x}_i^{a2} = \sum 
	...
	type. For 	calculating those features, first we expand (\ref{eq:4:agg_features1}):
	\begin{align}\label{eq:4:agg_features2}
		TRX_{agg}&(\mathcal{S}  ,i, t_p, cond_1, cond_2) = \bigg\{ x_l^{amt} \bigg\vert \nonumber \\  
		& \left(x_l^{id}=\mathbf{x}_i^{id}\right) \wedge  \left(hours(\mathbf{x}_i^{time},x_l^{time})< 
    t_p\right) \wedge \nonumber \\ 	
    &  \left(x_l^{cond_1} = \mathbf{x}_i^{cond_1}\right) \wedge	\left(x_l^{cond_2} = 
		\mathbf{x}_i^{cond_2}\right) \bigg\}_{l=1}^N.
		\end{align}
	Then, the features are calculated as:
	\[ \mathbf{x}_i^{a3} = \vert TRX_{agg}(\mathcal{S},i, t_p, country, type) \vert \quad \text{and} 
\]
	\[  \mathbf{x}_i^{a4} = \sum 	TRX_{agg}(\mathcal{S},i, t_p,country, type).	\]		
	...
	($\mathbf{x}_i^{a1}$, $\mathbf{x}_i^{a2}$, $\mathbf{x}_i^{a3}$ and $\mathbf{x}_i^{a4}$), by 
setting $t_p=24$. It is observed, how the 
	...
	features 	using (\ref{eq:4:agg_features1}), and also using (\ref{eq:4:agg_features2}) with the 
...
$\mathbf{x}_i^{a4}$\\
  ...
  \end{table}
	...
	to make the mistake of using the arithmetic mean. Because, as shown in 
  \figurename{~\ref{fig:4:von1}}, the arithmetic mean does not takes into account the periodic 
  behavior of the time feature. For example, the arithmetic mean of transaction time of four 
  transactions made at 2:00, 3:00, 22:00 and 23:00 is 12:40, which is counter intuitive since no 
  transaction was made close to that time. 
	...
	\begin{align}\label{eq:4:pe_features1}
		...
		\left(x_l^{id}=\mathbf{x}_i^{id}\right)\wedge  \nonumber \\ 
    & \left(hours(\mathbf{x}_i^{time},x_l^{time})< t_p\right) \bigg\}_{l=1}^N.
	\end{align}
	...
	\begin{equation}
	 \mathbf{x}_i^{time} \sim vonmises\left(\mu_{vM}(TRX_{vM}(\mathcal{S},i, t_p)), 
...
distribution]{\includegraphics[width=5.5cm]{ch4_fig_1c}\label{fig:4:von1}}
		\hfill
		\subfloat[Fitted von Mises 
distribution]{\includegraphics[width=5.5cm]{ch4_fig_2c}\label{fig:4:von2}}
		\\
		\subfloat[Expected time of 
transaction]{\includegraphics[width=5.5cm]{ch4_fig_3c}\label{fig:4:von3}}
  ...
  \end{figure} 
	...
	In \figurename{ \ref{fig:4:von2}}, the von Mises distribution calculation is shown.	Lastly, using 
  the estimated distribution a new set of features can be develop, ie. a binary feature 
  ($\mathbf{x}_i^{p1}$) if a new transaction time is within the confidence interval range with 
   ...
   \multicolumn{2}{|c|}{\textbf{Raw features}} & \multicolumn{4}{c|}{\textbf{Periodic features}} 
		\\  \hline
   \textbf{Id} & \textbf{Time} & $\mu$ & $\mu_{vM}$ & \textbf{Periodic CI} & $\mathbf{x}_i^{p1}$ \\
   \hline
		1& 01/01 18:20& --- & ---	 &	--- & ---\\
		2& 01/01 20:45& --- & ---	 &---	 & ---\\
   ...
   \end{table}
	...
	we calculate a feature $\mathbf{x}_i^{p1}$, as a binary feature that takes the value of one if 
  the current time of the transaction is within the confidence interval of the time of the previous 
	transactions with a confidence of $\alpha=0.9$. The example is show in 
  \tablename{~\ref{tab:4:agg_features_example2}}. In this example, it is shown, how the arithmetic 
  and periodic means differ, as for the last transaction both means are significantly different. 
  Moreover, the new feature, help to get a felling of when a customer is expected to make 
  transactions.
...
since the 
  ...
  \begin{equation}
   F_k(t)=\frac{1}{n_k}\vert \{ ( \mathbf{x}_i,y_i ) \in D_k \vert \hat p_i \le t \}\vert, \text{ 
for } 
   k\in\{0,1\}.
  \end{equation}
