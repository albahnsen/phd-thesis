...
t_p\right) \nonumber 
		\\ 	&  \wedge \left(x_l^{cond_1} = \mathbf{x}_i^{cond_1}\right) \wedge	\left(x_l^{cond_2} = 
...
group. 
   ...
   \textbf{TrxId} &\textbf{Time} & \textbf{Type} & \textbf{Country} & 
	\textbf{Amount} 		& $\mathbf{x}_i^{a1}$& $\mathbf{x}_i^{a2}$ & $\mathbf{x}_i^{a3}$ & 
$\mathbf{x}_i^{a4}$\\
   \hline
...
transaction]{\includegraphics[width=5.5cm]{ch4_fig_3c}\label{fig:von3}}
  ...
  \end{figure} 
	...
	feature ($\mathbf{x}_i^{p1}$) if a new transaction time is within the confidence interval range 
with 
   ...
   \textbf{Id} & \textbf{Time} & \textbf{Arithmetic mean} & \textbf{Periodic mean} & 
		\textbf{Periodic CI} & $\mathbf{x}_i^{p1}$ \\
   \hline
		1& 01/01/15 18:20& --- & ---	 &	--- & ---\\
		2& 01/01/15 20:45& --- & ---	 &---	 & ---\\
   ...
   \end{table}
	...
	we calculate a feature $\mathbf{x}_i^{p1}$, as a binary feature that takes the value of one if the 
current 
