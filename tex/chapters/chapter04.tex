...
\todo{Update with corrections in paper}
  ...
  is expected to make transactions at similar hours. We proposed a method for creating features 
...
\left(x_l^{id}=x_i^{id}\right) \nonumber \\
  ...
  amount of transaction $i$, $x_i^{id}$  the customer identification number of   transaction $i$, 
	...
	\[
		x_i^{a1} = \vert TRX_{agg}(\mathcal{S},i, t_p) \vert \quad  \text{and} \quad 
		x_i^{a2} = \sum TRX_{agg}(\mathcal{S},i, t_p).
	\]
	...
	several values, and the combination of combination criteria can be quite huge as well. In 
  \citep{CorreaBahnsen2013}, we used a total of 280 aggregated features. In particular we set the 
...
transaction]{\includegraphics[width=5.5cm]{ch4_fig_3c}\label{fig:4:von3}}
  ...
  ($x_i^{p1}$) if a new transaction time is within the confidence interval range with 
   ...
   \multicolumn{2}{|c|}{\textbf{Raw features}} & \multicolumn{4}{c|}{\textbf{Periodic features}} 
		\\  \hline
   \textbf{Id} & \textbf{Time} & $\mu$ & $\mu_{vM}$ & \textbf{Periodic CI} & $x_i^{p1}$ \\
   \hline
		1& 01/01 18:20& --- & ---	 &	--- & ---\\
		2& 01/01 20:45& --- & ---	 &---	 & ---\\
   ...
   \end{table}
	...
	we calculate a feature $x_i^{p1}$, as a binary feature that takes the value of one if 
  ...
  Moreover, the new feature, help to get a feeling of when a customer is expected to make 
  transactions.
